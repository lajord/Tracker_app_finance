...
export default function SettingsPage() {
  const { accounts, transactions, isSupabaseConfigured, isLoading, initializationError } = useFinanceStore();
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-400">Toutes les donnees sont gerees via Supabase.</p>
      </div>

      <Card>
        <CardHeader title="Etat de la connexion" />
        <div className="space-y-3 text-sm">
          <div className="flex flex-col gap-3 rounded-xl bg-zinc-900/50 p-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Card>
          ...
          <div className="space-y-1.5">
            {availableCategories.map((categoryName) => (
              <div key={categoryName} className="rounded-lg bg-zinc-900/40 px-3 py-2 text-sm text-zinc-100">
                {categoryName}
              </div>
            ))}
          </div>
        ...
        <Card>
          ...
          <div className="space-y-1.5">
            {accounts.map((account) => (
              <div
                key={account.id}
                className="flex flex-col gap-1 rounded-lg bg-zinc-900/40 px-3 py-2 text-sm sm:flex-row sm:items-center sm:justify-between"
              >
      ...
      <Card>
        ...
        <div className="text-xs text-zinc-500">{transactions.length} transactions chargees depuis la base de donnees.</div>
      </Card>
    </div>
  );
}
